...
def write_file_unavailable(arg: str) -> None:
    with open('unavailable_pullable.txt', 'a', encoding='UTF-8') as f:
        f.write(f'{arg}\n')
    ...
    async def _check(self, session: aiohttp.ClientSession, username: str) -> None:
        async with session.head(f'https://www.tiktok.com/@{username}') as response:
            if response.status == 200 and len(username) > 2:
                ...
                write_file_unavailable("https://www.tiktok.com/@"+username)
            else:
                print(
                    '%s[AVAILABLE/BANNED] https://www.tiktok.com/@%s%s'
                    % ('\u001b[32;1m', username, '\u001b[0m')
                )
                write_file("https://www.tiktok.com/@"+username)
...
if __name__ == '__main__':
    system('cls && title TikTok Username Checker by Vleuyy github.com/Vleuyy')
